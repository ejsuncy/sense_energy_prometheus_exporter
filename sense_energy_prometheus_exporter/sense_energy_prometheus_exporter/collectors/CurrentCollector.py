...
class CurrentCollector(CustomCollector):
    currentMetric: GaugeMetricFamily
    ...
    def reset(self):
        self.currentMetric = GaugeMetricFamily(self.build_name("amps", self.subsystemPrefix),
                                              f"{self.subsystemPrefix} measurement in amps",
    ...
    def collect(self):
        ...
        try:
            ...
            logging.debug("Collecting data for current")
            currents = self.sense_client.currents

            for current in currents:
                labels = [current[label] for label in self.currentLabels]
                self.currentMetric.add_metric(
                    labels=labels, value=current.amps)
