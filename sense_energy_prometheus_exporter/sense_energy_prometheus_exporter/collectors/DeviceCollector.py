...
from ..client.sense import SenseClient, SenseDevice
from prometheus_client.core import GaugeMetricFamily, Counter, Gauge


class DeviceCollector(CustomCollector):
    deviceMetric: GaugeMetricFamily
    ...
    def reset(self):
        self.deviceMetric = GaugeMetricFamily(self.build_name("watts", self.subsystemPrefix),
                                              f"{self.subsystemPrefix} power usage in watts",
                                              labels=self.deviceLabels)
    ...
    def collect(self):
        ...
        try:
            ...
            logging.debug("Collecting data for devices")
            devices: [SenseDevice] = self.sense_client.devices

            for device in devices:
                labels = [device[label] for label in self.deviceLabels]
                self.deviceMetric.add_metric(
                    labels=labels, value=device.watts)
