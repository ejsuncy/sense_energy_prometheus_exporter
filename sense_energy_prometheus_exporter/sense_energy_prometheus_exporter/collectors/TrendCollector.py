...
from ..client.sense import SenseClient, SenseTrend
from prometheus_client.core import GaugeMetricFamily, Counter, Gauge


class TrendCollector(CustomCollector):
    trendMetric: GaugeMetricFamily
    ...
    def reset(self):
        self.trendMetric = GaugeMetricFamily(self.build_name("kWh", self.subsystemPrefix),
                                              f"{self.subsystemPrefix} measurement in kWh",
                                              labels=self.trendLabels)
    ...
    def collect(self):
        ...
        try:
            ...
            logging.debug("Collecting data for trends")
            trends: [SenseTrend] = self.sense_client.trends

            for trend in trends:
                labels = [trend[label] for label in self.trendLabels]
                self.trendMetric.add_metric(
                    labels=labels, value=trend.kwh)
