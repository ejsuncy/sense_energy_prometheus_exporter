...
from ..client.sense import SenseClient, SenseFrequency
from prometheus_client.core import GaugeMetricFamily, Counter, Gauge


class FrequencyCollector(CustomCollector):
    frequencyMetric: GaugeMetricFamily
    ...
    def reset(self):
        self.frequencyMetric = GaugeMetricFamily(self.build_name("hertz", self.subsystemPrefix),
                                              f"{self.subsystemPrefix} measurement in hertz",
                                              labels=self.frequencyLabels)
    ...
    def collect(self):
        ...
        try:
            ...
            logging.debug("Collecting data for frequency")
            frequencies: [SenseFrequency] = self.sense_client.frequencies

            for frequency in frequencies:
                labels = [frequency[label] for label in self.frequencyLabels]
                self.frequencyMetric.add_metric(
                    labels=labels, value=frequency.hertz)
